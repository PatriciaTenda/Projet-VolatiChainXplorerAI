"""
Script : python injection_articles_financiers.py
Projet : VolatichainXplorerAI
Date : 2025-06-20

Description :
    Ce script insère automatiquement tous les articles scrappés
    depuis des fichiers .json dans la collection MongoDB "articles_financiers".

Usage :
    python injection_articles_financiers.py
"""
...
articles_dir = "data/raw/articles_financiers"

def insert_articles(name_db: str, uri: str, name_collection: str):
    # Vérifier sur les articles existes
    if not os.path.exists(articles_dir):
        logger.error(f" Dossier introuvable : {articles_dir}")
        return
...
if __name__ == "__main__":
    insert_articles(MONGO_DB, URI, "articles_financiers")
    print("Avec succès")